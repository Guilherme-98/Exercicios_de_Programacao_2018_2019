...
struct Carro *inserir(struct Carro *inicio,struct Carro conteudo){
    ...
    struct Carro *aux_ant;
        if(inicio == NULL){
            ...
            aux = inicio;
            if(conteudo.preco < aux->preco){
                aux = (struct Carro*)malloc(sizeof(struct Carro));
                strcpy(aux->modelo,conteudo.modelo);
                strcpy(aux->marca,conteudo.marca);
                aux->preco = conteudo.preco;
                aux->prox = inicio;
            }
            aux = inicio;
            aux_ant = NULL;
            if(conteudo.preco > aux->preco){
                while(aux != NULL &&  aux->preco < conteudo.preco){
                    aux_ant = aux;
                    aux = aux->prox;
                }
